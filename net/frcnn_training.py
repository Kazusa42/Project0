...
class AnchorTargetCreator(object):
    ...
    def _calc_ious(self, anchor, bbox):
        ...
        # get the most possible gt for each pre-anchor
    ...
    def _create_label(self, anchor, bbox):
        ...
        # if smaller than threshold -> negative sample
        label[max_ious < self.neg_iou_thresh] = 0
        # if larger than threshold -> positive sample
...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        n = imgs.shape[0]  # number of images
        img_size = imgs.shape[2:]

        base_feature = self.faster_rcnn.extractor(imgs)

        # use rpn to get adjust params, score, suggest bbox and pre-bbox
        ...
        for i in range(n):
            ...
            roi_cls_loc, roi_score = self.faster_rcnn.head(torch.unsqueeze(feature, 0),
                                                           sample_roi, sample_roi_index, img_size)

            n_sample = roi_cls_loc.size()[1]
            roi_cls_loc = roi_cls_loc.view(n_sample, -1, 4)
            roi_loc = roi_cls_loc[torch.arange(0, n_sample), gt_roi_label]

            # calculate the regression loss and classification loss
